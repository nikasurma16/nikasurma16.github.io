...
# "all" puts the installed face on every word; "display" keeps it for the
# large type and leaves body copy on Inter.
FONT_SCOPE = "all"

FALLBACK_SANS = '"Inter","Helvetica Neue",Helvetica,Arial,system-ui,sans-serif'
FALLBACK_MONO = ('ui-monospace,SFMono-Regular,"SF Mono",Menlo,Consolas,'
                 '"Liberation Mono",monospace')

# A face served from Google Fonts, used when assets/fonts/ is empty. Set both
# to "" to leave the site on Inter alone.
GOOGLE_DISPLAY = "Space Grotesk"
GOOGLE_DISPLAY_SPEC = "Space+Grotesk:wght@300..700"


def google_fonts_url():
    fams = ["Inter:wght@200;300;400"]
    if GOOGLE_DISPLAY_SPEC:
        fams.append(GOOGLE_DISPLAY_SPEC)
    return ("https://fonts.googleapis.com/css2?"
            + "&".join("family=" + f for f in fams) + "&display=swap")


def font_scope_vars(family):
    """The extra variables that put a face on body copy as well as headings."""
    if FONT_SCOPE != "all":
        return ""
    return ('  --sans:"%s",%s;\n  --mono:"%s",%s;\n'
            % (family, FALLBACK_SANS, family, FALLBACK_MONO))


def render_font_css():
    """Write assets/css/font.css from whatever sits in assets/fonts/, and
    return its path. Local files win; otherwise the Google-hosted face is
    named; with neither, nothing is written and the site stays on Inter.

    Sources are listed best-first: a browser takes the first one it can use,
    and woff2 is a fraction of the size of an otf."""
    d = os.path.join(ROOT, "assets", "fonts")
    out = os.path.join(ROOT, "assets", "css", "font.css")
    order = {ext: i for i, (ext, _) in enumerate(FONT_FORMATS)}
    fmts = dict(FONT_FORMATS)
    faces = []
    if os.path.isdir(d):
        for fn in sorted(os.listdir(d)):
            ext = os.path.splitext(fn)[1].lower()
            if ext in fmts:
                faces.append((order[ext], fn, fmts[ext]))
    faces.sort()
    if not faces:
        if GOOGLE_DISPLAY:
            write("assets/css/font.css",
                  "/* generated by build.py — do not edit by hand */\n"
                  ":root{\n  --display:\"%s\",%s;\n%s}\n"
                  % (GOOGLE_DISPLAY, FALLBACK_SANS,
                     font_scope_vars(GOOGLE_DISPLAY)))
            return "assets/css/font.css"
        if os.path.exists(out):
            os.remove(out)
            print("   removed assets/css/font.css (no font files)")
        return None
    src = ",\n       ".join('url("../fonts/%s") format("%s")' % (fn, fmt)
                            for _, fn, fmt in faces)
    write("assets/css/font.css",
          "/* generated by build.py from assets/fonts/ — do not edit by hand */\n"
          "@font-face{\n"
          "  font-family:\"Display\";\n"
          "  src:%s;\n"
          ...
          "}\n"
          ":root{\n  --display:\"Display\",%s;\n%s}\n"
          % (src, FALLBACK_SANS, font_scope_vars("Display")))
    return "assets/css/font.css"
...
{fontcss}<script>document.documentElement.classList.add('js')</script>
...
<a class="bg-watch" href="{p}{bgvideo}" target="_blank" rel="noopener">{watch}</a>
""".format(title=title, desc=desc, p=prefix, site=SITE_URL, og=og_image, bg=bg,
           ...
           bgvideo=asset("assets/video/%s.mp4" % bg),
           watch=t("watch the background", "смотреть фон"),
...
CV = [
 ("2026", '<b><a href="https://www.kunstmatrix.com/en/sikoraarts/" rel="noopener">Inaugural Exhibition for Abstract and Minimal Art 2026</a></b> — online exhibition, SikoraArts Online Gallery, Cortland, NY, 1 January — 21 February 2026',
          '<b><a href="https://www.kunstmatrix.com/en/sikoraarts/" rel="noopener">Inaugural Exhibition for Abstract and Minimal Art 2026</a></b> — онлайн-выставка, SikoraArts Online Gallery, Кортланд, Нью-Йорк, 1 января — 21 февраля 2026'),
